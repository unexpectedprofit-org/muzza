...
angular.module("Muzza.services").factory "StoreService", (days) ->
  ...
  class StoreDetailsObject
    constructor: ( data ) ->
      ...
      @displayHours = data.displayOpenHours
      @displayDays = _.keys(data.displayOpenHours)
      @dayOfTheWeek = new Date().getDay()
      @isOpenRightNow = ()->
        isOpened = false
        todayHourRanges = @displayHours[getDayName(@dayOfTheWeek)]
        ...
        if rightNow.getHours() > 0 and rightNow.getHours() < 6
          ...
          previousDay = (if @dayOfTheWeek is 0 then 6 else @dayOfTheWeek - 1)
          yesterdayHourRanges = @displayHours[getDayName( previousDay )]

          # if previous day was opened
          if yesterdayHourRanges.length > 1
        ...
        angular.forEach currentRanges, (range)->
          ...
          if rightNow >= open and rightNow <= closed then isOpened = true
